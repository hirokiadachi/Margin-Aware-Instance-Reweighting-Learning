...
def pgd(model, criterion, inputs, targets, alpha, epsilon, num_iters, training_type, lower=0, upper=1):
...
def training(epoch, model, dataloader, optimizer, alpha, epsilon, num_iters, gamma, beta,
    ...
    for batch_ind, (inputs, targets) in enumerate(dataloader):
        ...
        x = pgd(model, xent, inputs, targets, alpha, epsilon, num_iters, training_type, lower, upper)
